...
const ModalManageLot: React.FC<ModalManageLotProps> = ({
    ...
    try {
      ...
      if (selectedTracking) {
        ...
        if (!trackingResponse.ok) {
          ...
          throw new Error(
            errorData.error || "Failed to update tracking lot_id"
          );
        }
      }
  ...
  const handleUpdateTracking = async () => {
    ...
    const selectedTrackings = selectedRows.map(
      (index) => trackingData[index]
    );

    try {
      await Promise.all(
        selectedTrackings.map(async (tracking, idx) => {
          // Determine status based on latest filled date
          let newStatus = '';
          if (tracking.in_th) newStatus = 'in_th';
          else if (tracking.out_cn) newStatus = 'out_cn';
          ...
          if (!response.ok) {
            throw new Error(`Failed to update tracking for ID: ${tracking.tracking_id}`);
          }
        })
      );
  ...
  const handleTrackingSelect = async (id: string) => {
    try {
      ...
      if (tracking.lot_id && tracking.lot_id !== lotData.lot_id) {
        alert(
          `This tracking ID belongs to a different lot (Lot ID: ${tracking.lot_id}). You can't add it to this lot.`
        );
      } else {
  ...
  const handleRemoveFromLot = async () => {
    const selectedTrackings = selectedRows.map(
      (index) => trackingData[index].tracking_id
    );
  
    try {
      // First, remove the selected trackings
      await Promise.all(
        selectedTrackings.map(async (trackingId) => {
          ...
          if (!response.ok) {
            throw new Error(`Failed to remove tracking ${trackingId} from lot`);
          }
        })
      );
  
      // Then update the lot_order for remaining items
      const remainingTrackings = trackingData.filter(
        ...
        remainingTrackings.map(async (tracking, newIndex) => {
          const response = await fetch(
            `http://localhost:5001/tracking/${tracking.tracking_id}/updateFields`,
            {
              ...
              body: JSON.stringify({
                lot_order: newIndex + 1
              }),
            }
          );
  
          if (!response.ok) {
            throw new Error(`Failed to update lot_order for tracking ${tracking.tracking_id}`);
          }
        })
      );
      ...
      setSelectedRows([]);
      fetchTrackingData();
      alert("Selected items removed from lot successfully");
    } catch (error) {
      console.error("Error removing items from lot:", error);
      alert("Error removing items from lot");
    }
  };
    ...
    try {
      await Promise.all(
        selectedTrackings.map(async (trackingId) => {
          ...
          if (!response.ok) {
            throw new Error(`Failed to reset dates for tracking ${trackingId}`);
          }
        })
      );
      ...
      setSelectedRows([]);
      fetchTrackingData();
      alert("Dates reset successfully");
    } catch (error) {
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <div>
                            ...
                            >
                              {selectedTracking.image_item_paths.map(
                                (imagePath, index) => (
                                  <div
                                    key={index}
                                    ...
                                    onClick={() =>
                                      openImagePreview(
                                        selectedTracking.image_item_paths,
                                        index
                                      )
                                    }
                                  >
                        ...
                        <div>
                          ...
                          <div className="mb-3">
                            <label htmlFor="pricing" className="block mb-1">
                              คิดราคาตาม:
                            </label>
                            <select
                              id="pricing"
                          ...
                          <div className="mb-3">
                            <label htmlFor="user_rate" className="block mb-1">
                              เรท:
                            </label>
                            <select
                              id="user_rate"
                          ...
                          <div className="mb-3">
                            <label htmlFor="crate" className="block mb-1">
                              ตีลังไม้:
                            </label>
                            <select
                              id="crate"
                        ...
                        <div>
                          ...
                          <div className="mb-3">
                            <label htmlFor="in_cn" className="block mb-1">
                              เข้าโกดังจีน:
                            </label>
                            <input
                              id="in_cn"
                          ...
                          <div className="mb-3">
                            <label htmlFor="out_cn" className="block mb-1">
                              ออกโกดังจีน:
                            </label>
                            <input
                              id="out_cn"
                          ...
                          <div className="mb-3">
                            <label htmlFor="in_th" className="block mb-1">
                              เข้าโกดังไทย:
                            </label>
                            <input
                              id="in_th"
                        ...
                        <div>
                          ...
                          <div className="mb-3">
                            <label htmlFor="transport" className="block mb-1">
                              ค่าขนส่งจีน:
                            </label>
                            <input
                              id="transport"
                          ...
                          <div className="mb-3">
                            <label htmlFor="price_crate" className="block mb-1">
                              ค่าตีลัง:
                            </label>
                            <input
                              id="price_crate"
